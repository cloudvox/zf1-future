...
class Zend_Ldap_Collection_Iterator_Default implements Iterator, Countable
{
    ...
    public function current()
    {
        ...
        while ($name) {
            ...
            $entry[$attrName] = $data;
            trigger_error("PHP 7.2 Compatibility Alert:\n\tERROR: The \"ber_identifier\" parameter for function ldap_next_attribute() is removed since PHP 5.2.4".sprintf(" (%s::%s)", __FILE__, __LINE__)."\n\t".implode("\n\t", array_map(function ($item) { return sprintf("%s::%s", $item['file'], $item['line']); }, debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS))), E_USER_WARNING);
